(RaiselyComponents) => {
			...
			<div className="country-select">
				...
				>
					{Object.values(countries).map(({ code: country, flag }) => (
						<option value={country.toUpperCase()}>{country.toUpperCase()}</option>)
					)}
			...
			<React.Fragment>
				...
				<div className="highlight-actions">
					<button
						...
						onClick={() => setHighlight(some(
							// ensure same org can be chosen twice in a row
							data.filter(cause => cause.title !== org.title)
						))}
					>
	...
	return (props) => {
			...
			"AU": [
				{
					...
					logo: "https://d3n8a8pro7vhmx.cloudfront.net/alsnswact/sites/2/meta_images/original/site-logo.png?1553758710",
					description: "Legal services in NSW and the ACT",
					donateUrl: "https://www.alsnswact.org.au/donate"
				},
				{
					title: "ALRM",
					logo: "https://www.alrm.org.au/wp-content/uploads/2018/05/newlogo_small.jpg",
					description: "Legal Services SA",
					donateUrl: "https://www.alrm.org.au/donate/"
				},
				{
					title: "ALS WA",
					logo: "https://www.als.org.au/wp-content/uploads/2018/02/ALSWA-Logo-SM.jpg",
					description: "Legal Services WA",
					donateUrl: "https://www.als.org.au/donations-bequests/"
				},
				{
					title: "Pay the Rent",
					logo: "https://paytherent.net.au/wp-content/uploads/2020/01/Logo_circle_final-e1579760881379.png",
					description: "We live, work and play on land that was forcibly taken from Aboriginal people.  There has been no Treaty with the First Nations of this place and the effects of colonisation continue to this day.",
			...
			"US": [
				{
					title: "US Based Org",
					logo: null,
					description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Qui convenit? Itaque hic ipse iam pridem est reiectus; Quicquid enim a sapientia proficiscitur, id continuo debet expletum esse omnibus suis partibus; Tibi hoc incredibile, quod beatissimum. Duo Reges: constructio interrete.",
					donateUrl: "#"
				},
				{
					title: "US Based Charity",
					logo: "https://www.alrm.org.au/wp-content/uploads/2018/05/newlogo_small.jpg",
					description: "Quae quidem sapientes sequuntur duce natura tamquam videntes; Duo Reges: constructio interrete.",
					donateUrl: "#"
				},
			],
			"rest": [
		...
		const current = countries && detectedCountry && countries.find(c => c.code.toUpperCase() === detectedCountry) || { "rest": true, "title": defaultTitle, "sheet": defaultSheet };
		...
		React.useEffect(() => {
			...
			if (editor) {
				...
				setCountryData(dummyData[detectedCountry] || dummyData.rest);
				// setCountryData(data.detectedCountry || data.rest);
			}
		}, [detectedCountry]);
		...
		React.useEffect(() => {
			...
			if (current && countryData) setHighlight(
				(
					current.highlight &&
					countryData.find(cause => (
						cause.title.toLowerCase() === current.highlight.toLowerCase()
					))
				)
				|| some(countryData)
			)
